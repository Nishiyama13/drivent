...
describe('PUT /booking/:bookingId', () => {
  it('should respond with status 401 if no token is givin', async () => {
    const validBody = await buildValidBody(1);
    const response = await server.put(`/booking/${validBody.roomId}`).send(validBody);
    expect(response.status).toEqual(httpStatus.UNAUTHORIZED);
  });
  ...
  describe('When token is valid', () => {
    ...
    it('should respond with status 404 if user has not booking', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const invalidBody = {
        roomId: 666,
      ...
      const response = await server
        .put(`/booking/${invalidBody.roomId}`)
        .set('Authorization', `Bearer ${token}`)
        .send({ roomId: invalidBody.roomId });
